...
"""
NOTE:
Updated version of the MacheteV8 strategy, with migration from freqtrade V2 to V3. 
I still have work to do on this strategy, but it's a good start. 

Hyperopt is working, but there are still some minor problems.
If you want to hyperopt buy and sell params, 
there is an error with this: bb_trailing = self.custom_trade_info[trade.pair][self.cstp_bb_trailing_input.value].loc[current_time][self.cstp_bb_trailing_input.value]. 
I'm currently working on it, but if you have any ideas, feel free to share them with me.

I added a function to get current_price_low because there was an error when you go live and the bot takes a trade. 
You get an error in adjust_min_max_rates() because it must take two parameters, but it originally had only one. 
The missing parameter is current_rate_low. So, I did this: active_trade[0].adjust_min_max_rates(current_rate, current_rate) to avoid the error, 
but I'm not sure it's the best way to do it. I'm working on how to calculate the current_rate_low, but you can use it like that for now.
...
class MacheteV8b(IStrategy):
    ...
    buy_params = {
      "buy_should_use_get_buy_signal_adx_momentum": False,
      "buy_should_use_get_buy_signal_adx_smas": True,
      "buy_should_use_get_buy_signal_asdts_rockwelltrading": False,
      "buy_should_use_get_buy_signal_averages_strategy": False,
      "buy_should_use_get_buy_signal_awesome_macd": True,
      "buy_should_use_get_buy_signal_fisher_hull": False,
      "buy_should_use_get_buy_signal_gettin_moist": True,
      "buy_should_use_get_buy_signal_hlhb": True,
      "buy_should_use_get_buy_signal_macd_strategy": False,
      "buy_should_use_get_buy_signal_macd_strategy_crossed": False,
      "buy_should_use_get_buy_signal_pmax": False,
      "buy_should_use_get_buy_signal_quickie": True,
      "buy_should_use_get_buy_signal_scalp": True,
      "buy_should_use_get_buy_signal_simple": False,
      "buy_should_use_get_buy_signal_strategy001": False,
      "buy_should_use_get_buy_signal_technical_example_strategy": False,
      "buy_should_use_get_buy_signal_tema_rsi_strategy": False
    }

    # Sell hyperspace params:
    sell_params = {'cstp_bail_how': 'roc', 
                   'cstp_bail_roc': -0.032, 
                   'cstp_bail_time': 1108, 
    ...
    def get_buy_signal_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['ZLEMA'] = dema(dataframe, period=10)
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, **kwargs) -> float:
        ...
        # If in backtest or hyperopt, get the indicator values out of the trades dict (Thanks @JoeSchr!)
            ...
            # print(self.custom_trade_info[trade.pair][self.cstp_bb_trailing_input.value])
    ...
    def populate_trades(self, pair: str) -> dict:
        ...
        if self.config['runmode'].value in ('live', 'dry_run'):
            ...
            if active_trade:
                ...
                current_price_low = self.get_current_price_low(pair)
                print(f"Current rate: {current_rate}, current_price_low: {current_price_low}")
